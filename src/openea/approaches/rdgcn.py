...
class Layer:
    ...
    def build(self):
        ...
        self.rel_embeds = dual_X_2
        self.ent_embeds = output_layer
...
class RDGCN(BasicModel):
    ...
    def init(self):
        ...
        # NeoEA loss
        # ensure the ent and rel embeddings have the same dimensionality
        self.ent_embeds, self.rel_embeds = tf.nn.relu(self.gcn_model.ent_embeds), \
                tf.layers.dense(self.gcn_model.rel_embeds, int(self.gcn_model.ent_embeds.shape[-1]), tf.nn.relu)
        self.neoa = NeuralOntologyAlignment(args, kgs, self)
        # embed_loss denotes wd loss referred to the embeddings
        self.loss += self.neoa.embed_loss
        self.optimizer = tf.train.AdamOptimizer(self.args.learning_rate).minimize(self.loss, var_list=self.neoa.kge_vars)
            
        
        self.model_init = tf.global_variables_initializer()
        self.session = self.sess = load_session()
    ...
    def trainning(self):
        ...
        for i in range(1, self.args.max_epoch + 1):
            ...
            # NeoEA step
            self.neoa.run_one_epoch(self.feeddict)
            self.feeddict.update(self.neoa.collect())
    ...
    def test(self, save=True):
        ...
        rest_12, _, _, ranks = test(embeds1, embeds2, None, self.args.top_k, self.args.test_threads_num,
                             metric=self.args.eval_metric, normalize=self.args.eval_norm, csls_k=0, accurate=True, return_ranks=True)

        test(embeds1, embeds2, None, self.args.top_k, self.args.test_threads_num,
             metric=self.args.eval_metric, normalize=self.args.eval_norm, csls_k=self.args.csls, accurate=True, return_ranks=True)
        if save:
        ...
        import pandas as pd
        pd.Series(ranks).to_csv('%s_ranks.csv' % self.args.embedding_module)
        return ranks
